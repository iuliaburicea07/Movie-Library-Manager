...
export default function WatchedPage() {
  ...
  const filtered = watched.filter(w => filter === 'all' || w.type === filter);
    ...
    <>
      ...
      <main className="main" style={{ paddingTop: 'var(--header-h)' }}>
        <div className="watched-header">
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 32 }}>Watch List</h1>
        ...
        {filtered.length === 0 ? (
          <EmptyState
            title="Nothing in your watch list"
            text="Add movies and shows to your watch list to see them here"
          />
        ) : (
          <div className="section">
            <CardGrid
              ...
              onPlay={(item) => {
                const w = watched.find(x => x.id === item.id);
                setSelected({ item, type: w?.type || 'movie' });
              }}
              onStar={(item) => {
                const w = watched.find(x => x.id === item.id);
                if (w) toggleWatched(w);
              }}
              isStarred={() => true}
            />
          </div>
